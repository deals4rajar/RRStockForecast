...
@st.cache
def load_model1():
	  return joblib.load("Model/Bitcoin_Price_prediction_Model.sav")
...
st.subheader('Bitcoin/Stock Line Chart')
...
for i in range(base_days, len(m)+future_days):
    ...
    m = np.append(m ,pred)
    z = np.append(z, pred)
st.subheader('Predicted Future Days Bitcoin/Stock Price for 30 days')
z = np.array(z)
